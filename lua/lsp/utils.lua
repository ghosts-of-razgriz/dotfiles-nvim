...
function M.set_lsp_keymap(bufnr)
	...
	set_buf_map(bufnr, 'n', '\\d', '<cmd>lua vim.lsp.buf.definition()<cr>', settings)
	set_buf_map(bufnr, 'n', '\\i', '<cmd>lua vim.lsp.buf.implementation()<cr>', settings)
	set_buf_map(bufnr, 'n', '\\f', '<cmd>Telescope lsp_references<cr>', settings)
	set_buf_map(bufnr, 'n', '\\r', '<cmd>lua vim.lsp.buf.rename()<cr>', settings)
	set_buf_map(bufnr, 'n', '\\y', '<cmd>lua vim.lsp.buf.type_definition()<cr>', settings)
	set_buf_map(bufnr, 'n', 'K', '<cmd>lua vim.lsp.buf.hover()<cr>', settings)
	set_buf_map(bufnr, 'n', '<c-n>', '<cmd>lua vim.diagnostic.goto_next()<cr>', settings)
	set_buf_map(bufnr, 'n', '<c-p>', '<cmd>lua vim.diagnostic.goto_prev()<cr>', settings)
	set_buf_map(bufnr, 'n', '\\a', '<cmd>lua vim.lsp.buf.code_action()<cr>', settings)
	set_buf_map(bufnr, 'i', '<c-x><c-x>', '<cmd>lua vim.lsp.buf.signature_help()<cr>', settings)
end
